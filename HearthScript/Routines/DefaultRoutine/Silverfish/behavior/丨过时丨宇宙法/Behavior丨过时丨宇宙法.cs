...
{
    public partial class Behavior丨过时丨宇宙法 : Behavior
    {
        ...
        public override string BehaviorName() { return "丨过时丨宇宙法"; }
        ...
        public override int getComboPenality(CardDB.Card card, Minion target, Playfield p, Handmanager.Handcard nowHandcard)
        {
            ...
            switch (card.nameCN)
            {
				case CardDB.cardNameCN.卑劣的脏鼠:
                    ...
                    break;
				case CardDB.cardNameCN.露娜的口袋银河:	
					pen -= 200;
					break;
				case CardDB.cardNameCN.神奇的雷诺:
					if (p.ownHero.Hp <= 12 && p.enemyMinions.Count > 1) //有被斩杀风险
						pen -= 50 + (12 - p.ownHero.Hp) * (12 - p.ownHero.Hp);
					break;
				case CardDB.cardNameCN.雷诺杰克逊:
					if(p.ownHero.Hp <= 15)pen -= 100;
					else pen += 500;
					break;
			
				//疯狂的科学家
                case CardDB.cardNameCN.疯狂的科学家:
                    if (!hasSecretHead) pen -=3; 
					break;	
				//火焰结界
                case CardDB.cardNameCN.火焰结界:
                    ...
                    break;
                case CardDB.cardNameCN.寒冰屏障://寒冰屏障
                    pen -= 100;
                    break;
				case CardDB.cardNameCN.观星者露娜:
					pen += 10;
					break;
				case CardDB.cardNameCN.始祖龟朝圣者:
					pen -= 10;
					break;
				case CardDB.cardNameCN.星术师索兰莉安:
				case CardDB.cardNameCN.深水唤醒师:
				case CardDB.cardNameCN.奇利亚斯:
				case CardDB.cardNameCN.触手恐吓者:
				case CardDB.cardNameCN.洛欧塞布:
				case CardDB.cardNameCN.卡塔图防御者:
				case CardDB.cardNameCN.莫尔葛熔魔:
				case CardDB.cardNameCN.废料场巨像:
					pen -=5;
					break;
				case CardDB.cardNameCN.恩佐斯:
					pen += 20;
					break;
				case CardDB.cardNameCN.了不起的杰弗里斯:
					pen += 10;
					break;
			}
			return pen;
