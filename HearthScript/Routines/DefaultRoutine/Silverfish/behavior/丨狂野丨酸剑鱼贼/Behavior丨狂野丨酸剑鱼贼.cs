...
{
    public partial class Behavior丨狂野丨酸剑鱼贼 : Behavior
    {
        private int bonus_enemy = 4;
        private int bonus_mine = 4;

        public override string BehaviorName() { return "丨狂野丨酸剑鱼贼"; }
        ...
        public override int getComboPenality(CardDB.Card card, Minion target, Playfield p, Handmanager.Handcard nowHandcard)
        {
            ...
            switch (card.nameCN)
            {
                ...
                case CardDB.cardNameCN.剑鱼:  //有刀赶紧下
                    return -100;
                case CardDB.cardNameCN.暗影之门:                        //不缺手牌不要下
                ...
                case CardDB.cardNameCN.洛欧塞布:
                    if (p.ownMaxMana >= 5 && p.ownMinions.Count >= 3) return -100;
                        return 0;
